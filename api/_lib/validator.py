"""Feasibility validator — identical to backend version, lxml only."""
import re
...
class FeasibilityValidator:
    def __init__(self, thickness: float):
        ...
        self.min_hole_diameter = 1.0 * thickness
        self.min_slot_width = 1.0 * thickness
        self.min_bridge_width = 1.5
        self.min_corner_radius = 0.5

    def detect_islands(self, svg_content: str) -> Dict[str, Any]:
        try:
            root = etree.fromstring(svg_content.encode("utf-8"))
        except Exception:
            return {"ok": True, "island_count": 0, "issues": []}
        all_paths = root.findall(".//{http://www.w3.org/2000/svg}path") + root.findall(".//path")
        island_count = len(all_paths)
        issues = []
        if island_count > 20:
            issues.append(f"{island_count} losse paden gedetecteerd — controleer op fragiele eilanden.")
    ...
    def check_thin_features(self, svg_content: str) -> Dict[str, Any]:
        ...
        for i, path in enumerate(all_paths):
            ...
            if len(coords) < 4:
                issues.append(f"Pad #{i+1} heeft zeer weinig coordinaten — mogelijk te klein detail.")
